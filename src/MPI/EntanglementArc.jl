...
L = 30
px = [0.;0.1;0.2;0.3;0.4:0.01:0.5]#[0.; 0.1; 0.2; 0.3; 0.4:0.01:0.5...]#0:0.01:0.5
method = "mixed"
basis = :Z
prep_cat = false
n = 1
...
header = "#" * "L: $(L), method: $(method), basis: $(basis), newstarts: $(newstarts), iterations: $(iterations), thermalization: $(thermalization), between_measurements: $(between_measurements)"#join(["$(key): $(value)" for (key, value) in parameters], ", ")
...
if method == "pure"
    if prep_cat
        mkpath("data/pxpy/"*method*String(basis)*"/CAT/$(L)/$(newstarts)/EntanglementArc")
        pth = "data/pxpy/"*method*String(basis)*"/CAT/$(L)/$(newstarts)/EntanglementArc/" 
    else
        mkpath("data/pxpy/"*method*String(basis)*"/$(L)/$(newstarts)/EntanglementArc")
        pth = "data/pxpy/"*method*String(basis)*"/$(L)/$(newstarts)/EntanglementArc/" 
    end
else
    mkpath("data/pxpy/"*method*"/$(L)/$(newstarts)/EntanglementArc")
    pth = "data/pxpy/"*method*"/$(L)/$(newstarts)/EntanglementArc/" 
end
...
if RANK == 0 ##### MANAGER #####
    ...
    function main1(task)
        ...
        #do stuff
        try
            Sx, Sy = entanglement_arc([p,1-p,0], L;n = n, initialize = method, iterations = iterations, thermalization = thermalization, between_measurements = between_measurements, newstarts = newstarts)
            df = DataFrame(Sx = Sx,Sy= Sy)
...
RANK == 0 && @info timestr() *  "L = $(L), method = $(method), basis = $(basis)"
