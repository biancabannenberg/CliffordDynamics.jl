...
function calc_EE(state, l, L, dir::String)
    return entanglement_entropy(state, ss_compass(l,L, dir), Val(:rref))
end
...
function main(p, L;initialize = "mixed", basis = :Z, keep_result=false, phases=false, iterations = 2000, thermalization = 500, between_measurements = 200, newstarts = 1) 
    X,Y,Z = create_operators(L)
    N = L^2
