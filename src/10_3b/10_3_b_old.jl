...
#    L_unitcell * (x-1) + L_unitcell * L * (y-1) + L_unitcell * L^2 * (y-1)   --> iterate through all x,y and z with x in l+1:L, and y,z in 1:L
    function ss_10_3_b(lcols::Int, L::Int; cut = :xz)
        N = L^3 * L_unitcell
        if cut == :x1
        ...
        elseif cut == :z
            ...
            error("cut must be :x1, :x2, :xz, :y, :y2 or :z")
        end
    end
...
function create_operators(L)
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                YY[xi, yi, zi, 1] = genYY(N, index_xyz(xi, yi, zi, 1, L), index_xyz(xi, yi, zi, 3, L))
                YY[xi, yi, zi, 2] = genYY(N, index_xyz(xi, yi, zi, 2, L), index_xyz(xi, yi, zi, 4, L))
            end
        end
    end
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                ZZ[xi, yi, zi, 1] = genZZ(N, index_xyz(xi, yi, zi, 3, L), index_xyz(xi, yi, zi, 2, L))
                ZZ[xi, yi, zi, 2] = genZZ(N, index_xyz(xi, yi, zi, 4, L), index_xyz(xi, yi, zi + 1, 1, L))
            end
        end
    end
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                ...
                push!(is, index_xyz(xi, yi+1, zi, 3, L)); push!(ps, "Z")
                push!(is, index_xyz(xi, yi+1, zi, 1, L)); push!(ps, "Z")
                push!(is, index_xyz(xi+1, yi+1, zi-1, 4, L)); push!(ps, "Z")
                push!(is, index_xyz(xi+1, yi+1, zi-1, 2, L)); push!(ps, "X")
                push!(is, index_xyz(xi+1, yi+1, zi-1, 3, L)); push!(ps, "Y")
                push!(is, index_xyz(xi+1, yi, zi-1, 2, L)); push!(ps, "Z")
                ...
                push!(is, index_xyz(xi, yi+1, zi-1, 3, L)); push!(ps, "Y")
                push!(is, index_xyz(xi, yi, zi-1, 2, L)); push!(ps, "Z")
                ...
                push!(is, index_xyz(xi, yi, zi+1, 1, L)); push!(ps, "Y")
                ...
                push!(is, index_xyz(xi, yi-1, zi+1, 1, L)); push!(ps, "Y")
                push!(is, index_xyz(xi+1, yi-1, zi, 4, L)); push!(ps, "Z")
                push!(is, index_xyz(xi+1, yi-1, zi, 2, L)); push!(ps, "Z")
    ...
    for xi in 1:L
        ...
        push!(is, index_xyz(xi + 1, 1, 0, 4, L)); push!(ps, "Y")
    end
    ...
    ps = []
    for yi in 1:L
        push!(is, index_xyz(1, yi, 1, 2, L)); push!(ps, "Y")
        push!(is, index_xyz(1, yi+1, 1, 3, L)); push!(ps, "Y")
    end
...
function simulate(L::Int, p; cut = :y, iterations::Int = 25, thermalization_steps = 20, keep_result = false, phases = false)
    ...
    for _ in 1:iterations
        ...
        if cut == :all
            ...
            EE_y2 += calc_EE(state, Int(L//2), L, cut = :y2)
            Tmi_y2 += TMI(state, L, cut = :y2)
    ...
    if cut == :all
        return EE_x1/iterations, Tmi_x1/iterations, EE_x2/iterations, Tmi_x2/iterations, EE_xz/iterations, Tmi_xz/iterations, EE_y/iterations, Tmi_y/iterations, EE_y2/iterations, Tmi_y2/iterations, EE_z/iterations, Tmi_z/iterations
    else
