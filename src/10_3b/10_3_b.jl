...
#    L_unitcell * (x-1) + L_unitcell * L * (y-1) + L_unitcell * L^2 * (y-1)   --> iterate through all x,y and z with x in l+1:L, and y,z in 1:L
    function ss_10_3_b(lcols::Int, L::Int; cut = :a3)
        N = L^3 * L_unitcell
        if cut == :a1
        ...
        elseif cut == :z1 ### cut along first z-bond of unit cell (between 1,3)
            if lcols == L
                return ([1:N...])
            elseif lcols < L
                return ([[(L_unitcell * (j-1) + L_unitcell * L * (k-1) + L_unitcell * L^2 * (m-1) + l)
                    ...
                    for l in [3;2;4]       # select atoms 2 through 6 of the unit cell
                    ]...;
        ...
        elseif cut == :z2 ### cut along second z-bond of unit cell (between 2,4)
            if lcols == L
                return ([1:N...])
            elseif lcols < L
                return ([[(L_unitcell * (j-1) + L_unitcell * L * (k-1) + L_unitcell * L^2 * (m-1) + l)
                    ...
                    for k in 1:L           # iterate over the y-direction
                    for m in lcols+1       # iterate over the z-direction
                    for l in 4:4       # select atoms 2 through 6 of the unit cell
                    ]...;
                    [(L_unitcell * (j-1) + L_unitcell * L * (k-1) + L_unitcell * L^2 * (m-1) + l)
                    for j in 1:L           # iterate over the x-direction
                    for k in 1:L           # iterate over the y-direction
        ...
        elseif cut == :y ### cut along y bonds (negative diagonal: a3,-a2, a1)
            if lcols == L
                return ([1:N...])
            elseif lcols < L
                is = []
                ...
                for xs  in 1:L
                    ...
                    for ys in lcols+1:L
                        y = ys
                        for z in 1:L
                            push!(is, index_xyz(x, y, z, 1, L))
                            push!(is, index_xyz(x, y, z, 3, L))
                            y -= 1
                            push!(is, index_xyz(x, y, z, 2, L))
                            push!(is, index_xyz(x, y, z, 4, L))
                            x += 1
                        end
                    end
                end
            ...
            error("cut must be :a1, :a2, :a3, :z1, :z2 or :y")
        end
    end

    function ss_10_3_b(lcols, L::Int; cut = :xz)
        N = L^3 * L_unitcell
        if cut == :a1 ## here one only cuts red bonds == XX 
        ...
        elseif cut == :a2 ## here one only cuts red bonds == XX
        ...
        elseif cut == :a3 ## here one cuts red+green bonds == XX+YY
        ...
        elseif cut == :z1 ### cut along first z-bond of unit cell (between 1,3): only blue bonds == YY
            if lcols == 1:L
                return ([1:N...])
            else
                return ([[(L_unitcell * (j-1) + L_unitcell * L * (k-1) + L_unitcell * L^2 * (m-1) + l)
...
#firstly one need to define the subsystem of length l and total systemsize L (ss_10_3_b(l,L))
#state: MixedDestabilizer
    function calc_EE(state, l, L; cut = :x)
        return entanglement_entropy(state, ss_10_3_b(l,L, cut = cut), Val(:rref))
    end

#function that calculates entanglement for given subdivistion lcols
    function S_A(state, lcols, L; cut = :x)
        return entanglement_entropy(state, ss_10_3_b(lcols,L, cut = cut), Val(:rref))
    end
...
function create_operators(L)
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                XX[xi, yi, zi, 1] = genXX(N, index_xyz(xi, yi, zi, 1, L), index_xyz(xi + 1, yi, zi - 1, 4, L))
                XX[xi, yi, zi, 2] = genXX(N, index_xyz(xi, yi, zi, 2, L), index_xyz(xi, yi + 1, zi, 3, L))
            end
        end
    end
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                YY[xi, yi, zi, 1] = genYY(N, index_xyz(xi, yi, zi, 3, L), index_xyz(xi, yi, zi, 2, L))
                YY[xi, yi, zi, 2] = genYY(N, index_xyz(xi, yi, zi, 4, L), index_xyz(xi, yi, zi + 1, 1, L))
            end
        end
    end
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                ZZ[xi, yi, zi, 1] = genZZ(N, index_xyz(xi, yi, zi, 1, L), index_xyz(xi, yi, zi, 3, L))
                ZZ[xi, yi, zi, 2] = genZZ(N, index_xyz(xi, yi, zi, 2, L), index_xyz(xi, yi, zi, 4, L))
            end
        end
    end
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                ...
                push!(is, index_xyz(xi, yi, zi, 3, L)); push!(ps, "X")
                ...
                push!(is, index_xyz(xi+1, yi+1, zi-1, 4, L)); push!(ps, "Y")
                push!(is, index_xyz(xi+1, yi+1, zi-1, 2, L)); push!(ps, "X")
                push!(is, index_xyz(xi+1, yi+1, zi-1, 3, L)); push!(ps, "Z")
                push!(is, index_xyz(xi+1, yi, zi-1, 2, L)); push!(ps, "Y")
                push!(is, index_xyz(xi+1, yi, zi-1, 4, L)); push!(ps, "Y")
                ...
                is = []
                ps = []
                push!(is, index_xyz(xi, yi, zi, 1, L)); push!(ps, "X")
                push!(is, index_xyz(xi, yi, zi, 3, L)); push!(ps, "X")
                push!(is, index_xyz(xi, yi, zi, 2, L)); push!(ps, "Z")
                push!(is, index_xyz(xi, yi+1, zi, 3, L)); push!(ps, "Y")
                push!(is, index_xyz(xi, yi+1, zi, 1, L)); push!(ps, "X")
                push!(is, index_xyz(xi, yi+1, zi-1, 4, L)); push!(ps, "X")
                push!(is, index_xyz(xi, yi+1, zi-1, 2, L)); push!(ps, "X")
                push!(is, index_xyz(xi, yi+1, zi-1, 3, L)); push!(ps, "Z")
                push!(is, index_xyz(xi, yi, zi-1, 2, L)); push!(ps, "Y")
                push!(is, index_xyz(xi, yi, zi-1, 4, L)); push!(ps, "X")
                ...
                push!(is, index_xyz(xi, yi, zi, 1, L)); push!(ps, "Y")
                push!(is, index_xyz(xi, yi, zi, 3, L)); push!(ps, "X")
                push!(is, index_xyz(xi, yi, zi, 2, L)); push!(ps, "X")
                ...
                push!(is, index_xyz(xi+1, yi, zi-1, 4, L)); push!(ps, "Z")
                ...
                push!(is, index_xyz(xi, yi-1, zi, 2, L)); push!(ps, "Y")
                push!(is, index_xyz(xi, yi-1, zi, 4, L)); push!(ps, "X")
                push!(is, index_xyz(xi, yi-1, zi+1, 1, L)); push!(ps, "Z")
                push!(is, index_xyz(xi+1, yi-1, zi, 4, L)); push!(ps, "Y")
    ...
    ps = [] ### not redundant
    for xi in 1:L
        push!(is, index_xyz(xi, 1, 1, 1, L)); push!(ps, "Z")
        push!(is, index_xyz(xi + 1, 1, 0, 4, L)); push!(ps, "Z")
    end
    ...
    ps = []
    for yi in 1:L
        push!(is, index_xyz(1, yi, 1, 2, L)); push!(ps, "Z")
        push!(is, index_xyz(1, yi+1, 1, 3, L)); push!(ps, "Z")
    end
    ...
    ps = []
    for zi in 1:L
        push!(is, index_xyz(1, 1, zi, 1, L)); push!(ps, "X")
        push!(is, index_xyz(1, 1, zi, 2, L)); push!(ps, "X")
        push!(is, index_xyz(1, 1, zi, 3, L)); push!(ps, "X")
        push!(is, index_xyz(1, 1, zi, 4, L)); push!(ps, "X")
    end
...
function simulate(L::Int, p; cut = :y, iterations::Int = 25, thermalization_steps = 20, keep_result = false, phases = false)
    ...
    for _ in 1:iterations
        ...
        if cut == :all
            ...
            EE_z1 += calc_EE(state, Int(L//2), L, cut = :z1) 
            Tmi_z1 += TMI(state, L, cut = :z1)
            EE_z2 += calc_EE(state, Int(L//2), L, cut = :z2)
            Tmi_z2 += TMI(state, L, cut = :z2)
    ...
    if cut == :all
        return EE_a1/iterations, Tmi_a1/iterations, EE_a2/iterations, Tmi_a2/iterations, EE_a3/iterations, Tmi_a3/iterations, EE_z1/iterations, Tmi_z1/iterations, EE_z2/iterations, Tmi_z2/iterations, EE_y/iterations, Tmi_y/iterations
    else
