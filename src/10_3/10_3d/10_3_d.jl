...
#    L_unitcell * (x-1) + L_unitcell * L * (y-1) + L_unitcell * L^2 * (y-1)   --> iterate through all x,y and z with x in l+1:L, and y,z in 1:L
    function ss_10_3_a(lcols::Int, L::Int; cut = :y)
        ...
        #             for j in 1:L           # iterate over the x-direction
        #             for k in 1:L           # iterate over the y-direction
        #             for m in 1     # iterate over the z-direction
...
function create_operators(L)
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                YY[xi, yi, zi, 1] = genYY(N, index_xyz(xi, yi, zi, 4, L), index_xyz(xi+1, yi, zi, 1, L))
                YY[xi, yi, zi, 2] = genYY(N, index_xyz(xi, yi, zi, 3, L), index_xyz(xi, yi+1, zi-1, 2, L))
            end
        end
    end
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                ZZ[xi, yi, zi, 1] = genZZ(N, index_xyz(xi, yi, zi, 2, L), index_xyz(xi, yi+1, zi, 1, L))
                ZZ[xi, yi, zi, 2] = genZZ(N, index_xyz(xi, yi, zi, 3, L), index_xyz(xi, yi, zi-1, 4, L))
            end
        end
    end
    ...
    for xi in 1:L
        for yi in 1:L
            for zi in 1:L
                ...
                push!(is, index_xyz(xi, yi+2, zi-1, 1, L)); push!(ps, "X")
                push!(is, index_xyz(xi-1, yi+2, zi-1, 4, L)); push!(ps, "Z")
                push!(is, index_xyz(xi-1, yi+2, zi-1, 2, L)); push!(ps, "Z")
                push!(is, index_xyz(xi-1, yi+1, zi, 3, L)); push!(ps, "Z")
                push!(is, index_xyz(xi-1, yi+1, zi, 1, L)); push!(ps, "Y")
                push!(is, index_xyz(xi-1, yi, zi, 2, L)); push!(ps, "Y")
                ...
                push!(is, index_xyz(xi, yi, zi-1, 4, L)); push!(ps, "Y")
                ...
                push!(is, index_xyz(xi, yi-2, zi, 2, L)); push!(ps, "Y")
                push!(is, index_xyz(xi, yi-2, zi, 4, L)); push!(ps, "Y")
                push!(is, index_xyz(xi, yi-2, zi+1, 3, L)); push!(ps, "X")
                ...
                # push!(is, index_xyz(xi, yi+1, zi-1, 2, L)); push!(ps, "X")
                # push!(is, index_xyz(xi, yi+2, zi-1, 1, L)); push!(ps, "X")
                # push!(is, index_xyz(xi-1, yi+2, zi-1, 4, L)); push!(ps, "X")
                # push!(is, index_xyz(xi-1, yi+2, zi, 3, L)); push!(ps, "Y")
                # push!(is, index_xyz(xi-1, yi+2, zi, 1, L)); push!(ps, "Y")
                # push!(is, index_xyz(xi-1, yi+1, zi, 2, L)); push!(ps, "X")
                # push!(is, index_xyz(xi-1, yi, zi+1, 3, L)); push!(ps, "X")
                # push!(is, index_xyz(xi-1, yi, zi, 4, L)); push!(ps, "X")
                ...
                push!(is, index_xyz(xi, yi, zi+1, 3, L)); push!(ps, "X")     
                push!(is, index_xyz(xi, yi+1, zi, 2, L)); push!(ps, "X")        
                push!(is, index_xyz(xi, yi+2, zi, 1, L)); push!(ps, "X")  
                push!(is, index_xyz(xi-1, yi+2, zi, 4, L)); push!(ps, "Z")       
                push!(is, index_xyz(xi-1, yi+2, zi, 2, L)); push!(ps, "Z")     
                push!(is, index_xyz(xi-1, yi+1, zi+1, 3, L)); push!(ps, "X")       
    ...
    for xi in 1:L
        ...
        push!(is, index_xyz(xi, 1, 1, 3, L)); push!(ps, "Z")
        push!(is, index_xyz(xi, 2, 0, 2, L)); push!(ps, "Z")
        push!(is, index_xyz(xi, 2, 0, 4, L)); push!(ps, "Y")
        push!(is, index_xyz(xi, 2, 1, 3, L)); push!(ps, "Y")
        push!(is, index_xyz(xi, 2, 1, 1, L)); push!(ps, "Y")
        push!(is, index_xyz(xi, 1, 1, 2, L)); push!(ps, "Y")
        push!(is, index_xyz(xi, 1, 1, 4, L)); push!(ps, "Z")
    end
    ...
    ps = []
    for yi in 1:L
        push!(is, index_xyz(1, yi, 1, 3, L)); push!(ps, "Z")
        push!(is, index_xyz(2, yi, 0, 2, L)); push!(ps, "Z")
        push!(is, index_xyz(2, yi, 0, 4, L)); push!(ps, "Y")
        push!(is, index_xyz(2, yi, 1, 3, L)); push!(ps, "Y")
    end
    ...
    ps = []
    for zi in 1:L
        push!(is, index_xyz(1, 1, zi, 4, L)); push!(ps, "Y")
        push!(is, index_xyz(1, 1, zi + 1, 3, L)); push!(ps, "X")
        push!(is, index_xyz(1, 2, zi, 2, L)); push!(ps, "Z")
        push!(is, index_xyz(1, 2, zi, 4, L)); push!(ps, "Y")
        push!(is, index_xyz(1, 2, zi+1, 3, L)); push!(ps, "Y")
        push!(is, index_xyz(1, 2, zi+1, 1, L)); push!(ps, "Y")
        push!(is, index_xyz(1, 1, zi+1, 2, L)); push!(ps, "Y")
    end
