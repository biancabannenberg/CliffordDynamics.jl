...
iterations = 20
thermalization = 30
cut = [:a1, :a2, :a3, :x1, :x2, :xz]
cut_string = join(vcat("$(cut[1])",["_$(cut[i])" for i in 2:length(cut)]))
# iterations = 1
...
header = "#" * "L: $(L), cut: $(cut_string), iterations: $(iterations), thermalization: $(thermalization)" #join(["$(key): $(value)" for (key, value) in parameters], ", ")
...
pth = "data/phaseDiagram/$(L)/$(cut_string)/$(iterations)/" 
mkpath(pth)

simulate(L, [1.0, 0.0, 0.0]; cut = cut, iterations=iterations, thermalization_steps=thermalization)
...
if RANK == 0 ##### MANAGER #####
    ...
    function main1(task)
        ...
        #do stuff
        try
            data = simulate(L, p;cut = cut, iterations=iterations, thermalization_steps=thermalization)
...
RANK == 0 && @info timestr() *  "L = $(L), iterations = $(iterations), thermalization = $(thermalization)"
...
function manager(p::Int, n::Int; t0::DateTime)
    ...
    open(pth*"mean.csv", "w") do io
        ...
        # println(io, "p,EE_z,EE_y,EE_xz,TMI_z,TMI_y,TMI_xz")
        CSV.write(io, DataFrame(results), append = false, header = true)
    end
